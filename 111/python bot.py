...
async def send_message_to_admin(context: ContextTypes.DEFAULT_TYPE, user_id: int, message: str, file_id=None, file_type=None):
    ...
    try:
        ...
        if file_id:
            if file_type == 'photo':
                await context.bot.send_photo(chat_id=ADMIN_ID, photo=file_id, caption=f"📎 Чек об оплате от пользователя ID: {user_id}")
            elif file_type == 'document':
                await context.bot.send_document(chat_id=ADMIN_ID, document=file_id, caption=f"📎 Чек об оплате от пользователя ID: {user_id}")
        return True
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.user_data.get('admin_notified') and str(user.id) == str(ADMIN_ID):
        try:
            await context.bot.send_message(chat_id=ADMIN_ID, text="✅ Соединение с ботом установлено успешно! Теперь вы будете получать уведомления о новых оплатах.")
...
async def process_payment_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    payment_message += (
        "\n\nСтоимость курса может изменяться в зависимости от условий. "
        f"Пожалуйста, свяжитесь с администратором ({ADMIN_USERNAME}) для уточнения актуальной стоимости."
